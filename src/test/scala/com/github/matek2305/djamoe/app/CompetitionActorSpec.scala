...
class CompetitionActorSpec
  ...
  trait Test {
    implicit val makeBetPolicy: MakeBetPolicy = mock[MakeBetPolicy]
    when(makeBetPolicy.check(any)) thenReturn true
  ...
  "CompetitionActor" should {
    ...
    "prevent betting when policy check fails" in new Test {
      competitionActor ! sampleAddMatchCommand
      val matchId: MatchId = expectMsgType[CommandProcessed].event.matchId
      ...
      when(makeBetPolicy.check(any)) thenReturn false
